...
describe HeadMusic::Style::Guidelines::SustainAcrossBarlines do
  ...
  context "with a first-species line" do
    before do
      %w[A4 A4 G4 A4 B4 C5 C5 B4 D5 C#5 D5].each.with_index(1) do |pitch, bar|
    ...
    it "marks every middle bar beyond the tolerated breaks" do
      expect(guideline.marks_count).to eq 7
    end

    it "scores each excess break at the penalty factor" do
  ...
  context "with occasional breaks in the syncopation" do
    ...
    context "with breaks beyond the tolerated ratio" do
      ...
      it "marks only the breaks past the allowance" do
