...
describe HeadMusic::Style::Guidelines::OnePerBar do
  ...
  context "with one whole note in each middle bar" do
    before do
  ...
  context "with half notes in a middle bar" do
    before do
      counterpoint.place("1:1", :whole, "A4")
      # Bar 2: two half notes instead of one whole
      counterpoint.place("2:1", :half, "A4")
      counterpoint.place("2:3", :half, "C5")
      (3..11).each.with_index(3) do |_, bar|
