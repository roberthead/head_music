...
describe HeadMusic::Style::Annotations::DirectionChanges do
  ...
  context 'when the notes are all ascending' do
    before do
      %w[D4 E4 F4 G4 A4 B4 C5 D5].each_with_index do |pitch, bar|
  ...
  context 'when the direction changes only once' do
    before do
      %w[D4 E4 F4 G4 A4 G4 F4 E4 D4].each_with_index do |pitch, bar|
        voice.place("#{bar + 1}:1", :whole, pitch)
      end
    end
  ...
  context 'when the direction changes frequently' do
    before do
      %w[D4 E4 F4 G4 F4 A4 G4 B4 C5 D5].each_with_index do |pitch, bar|
        voice.place("#{bar + 1}:1", :whole, pitch)
      end
    end
