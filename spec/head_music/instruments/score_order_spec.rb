...
describe HeadMusic::Instruments::ScoreOrder do
  ...
  describe "#order" do
    ...
    context "with instrument variants" do
      let(:instruments) { %w[flute alto_saxophone tenor_saxophone alto_flute trumpet_in_c alto_recorder piccolo trumpet_in_eb] }

      it "orders variants correctly" do
        ordered = orchestral_order.order(instruments)
        expect(ordered.map(&:name)).to eq(["piccolo", "alto flute", "alto recorder", "alto saxophone", "tenor saxophone", "Trumpet in C", "Trumpet in Eb"])
      end
    end
