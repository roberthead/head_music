...
describe "HeadMusic::Instruments::GenericInstrument" do
  describe "get" do
    context "when using an alias name" do
      let(:piccolo_by_alias) { HeadMusic::Instruments::GenericInstrument.get("piccolo") }
      let(:piccolo_by_name) { HeadMusic::Instruments::GenericInstrument.get("piccolo_flute") }
      ...
      it "returns piano for 'pianoforte' alias" do
        instrument = HeadMusic::Instruments::GenericInstrument.get("pianoforte")
        expect(instrument.name).to eq("piano")
        expect(instrument.name_key).to eq(:piano)
      end

      it "returns cor anglais for 'English horn' alias" do
        instrument = HeadMusic::Instruments::GenericInstrument.get("English horn")
