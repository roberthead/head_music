# Renders a flow to LilyPond, re-parses the output, and asserts that
...
module LilyPondRoundTripHelper
  def expect_lily_pond_round_trip(flow)
    ...
    expect(reparsed.voices.map(&:role)).to eq flow.voices.map(&:role)
  ...
  def expect_equivalent_bar_changes(reparsed, flow)
    (flow.earliest_bar_number..flow.latest_bar_number).each do |bar_number|
      expect(reparsed.key_signature_at(bar_number)).to eq flow.key_signature_at(bar_number)
      expect(reparsed.meter_at(bar_number).to_s).to eq flow.meter_at(bar_number).to_s
    end
  end
