...
# Three kinds of corpus voice, and the difference matters to every sentence
# below that says "published". A ladder is generated by the capture script. A
# constructed counterexample is written into spec/ by hand to fail a rule on
# purpose -- the doubled-octave line is one cantus firmus sung twice -- and is
# no more published material than a ladder is. Calling either one a published
# fixture is the exact prose drift this script was split out to prevent.
#
# Both are named rather than inferred, because nothing in a row distinguishes
# them from a fixture: a constructed line grades like any other, and the ladders
# are only recognizable by the label the capture script gives them. The
# alternative, widening the ladder regex to swallow the counterexamples, would
# falsify the sentence after it -- the doubled-octave line is first-species
# material, and is not a ladder.
LADDER_ENTRIES = /\Aagainst-|\Asolo-/
CONSTRUCTED_ENTRIES = /\Adoubled_octave_examples-/

assessable_names = assessable.map { |row| row[:corpus] }.uniq
ladder_entries = assessable_names.grep(LADDER_ENTRIES)
constructed_entries = assessable_names.grep(CONSTRUCTED_ENTRIES)
published_entries = assessable_names - ladder_entries - constructed_entries

def rows_from(rows, entries)
  rows.count { |row| entries.include?(row[:corpus]) }
end
...
# The two multiplications are computed from their own captures. A story that
# adds a fixture grows the corpus between the first capture and the last, and
# reusing the first capture's entry count here prints an equation that does not
# multiply out -- the one arithmetic error a reader would catch by hand.
last_entries = last_capture.map { |row| row[:corpus] }.uniq.length
lines << "The capture script takes no arguments beyond its output path and asks only what " \
         ...
         "#{guides} registry entries — and **#{last_capture.length}** in the last, " \
         "#{last_entries} × #{last_capture.map { |row| row[:guide] }.uniq.length}. " \
         "An asterisk marks an unassessable voice."
lines << ""
provenance = [
  "**#{rows_from(assessable, published_entries)}** come from published fixtures " \
    "(#{voices(published_entries.length)})",
  (if ladder_entries.any?
     "**#{rows_from(assessable, ladder_entries)}** from #{voices(ladder_entries.length)} of " \
      "synthetic ladder-against-cantus"
   end),
  (if constructed_entries.any?
     "**#{rows_from(assessable, constructed_entries)}** from #{voices(constructed_entries.length)} " \
      "of constructed counterexample — written to fail a rule on purpose, and no more published " \
      "material than the ladders are"
   end)
].compact
lines << "**Two denominators live in the harmony numbers and must not be conflated.** Of the " \
         ...
         "#{provenance[0..-2].join(", ")}#{", and " if provenance.length > 1}" \
         "#{provenance.last}. Every assessable harmony row is " \
